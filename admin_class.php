...
Class Action {
	...
	function login() {
		...
		if ($qry->num_rows > 0) {
			...
			// Check if the selected role matches the user's type
			...
			if ($userType != 3) {
				// Send SMS for non-admin users
				$faculty_qry = $this->db->query("SELECT contact FROM faculty WHERE firstname = '{$username}'");
				if ($faculty_qry->num_rows > 0) {
					$contact = $faculty_qry->fetch_assoc()['contact'];
					// Send SMS using a third-party service
					$this->sendSMS($contact, "You have logged in to the console with username: {$username}");
				}
			}
